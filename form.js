...
const SUPABASE_ANON = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFld2dvamhvc3p6Y2JjanphcnZtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMxMDQ2MjAsImV4cCI6MjA4ODY4MDYyMH0.l3o_hYATusjOe1wv84f-TMMuDq73zdj1aqogxZ5WakI";
...
function clearStates() {
  form.querySelectorAll(".input-group, .phone-group, .custom-select-group").forEach((g) => {
...
function setBtnState(state) {
  const states = {
    ...
    success: { text: "Sent ✓",       disabled: true,  cls: "success"   },
...
phoneField.addEventListener("blur", () => {
  ...
  if (val && !isValidPhone()) {
    setError(phoneField, "Enter a valid phone number.");
  } else if (val) {
...
form.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    phone:        window.iti ? window.iti.getNumber() : phoneField.value.trim() || null,
  ...
  try {
    ...
    if (selectText) {
      selectText.textContent = "Project type";
